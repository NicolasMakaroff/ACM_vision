...
class CCV(nn.Module):
    ...
    def __init__(self, initial_contours, dt, lambda_, color=False):
        ...
        self.c, self.h, self.w = initial_contours.shape
        self.u1 = initial_contours
        self.dt = dt
        self.lambda_ = lambda_ * math.sqrt(torch.pi)/torch.sqrt(dt)
        self.color = color
    ...
    def __call__(self, input_tensor, maxIter = 100, plot = False):
        ...
        u2 = torch.FloatTensor(c,h,w).fill_(1)- self.u1
        ...
        while iter <= maxIter:
            ...
            if NS == 2 and NC == 1:
                ...
                index1 = f1 + (uh2.permute(1,2,0) * self.lambda_).permute(2,0,1)
                index2 = f2 + (uh1.permute(1,2,0) * self.lambda_).permute(2,0,1)
                ...
                index1 = f11 + f12 + f13 - (uh1.permute(1,2,0) * self.lambda_).permute(2,0,1)
                index2 = f21 + f22 + f23 - (uh2.permute(1,2,0) * self.lambda_).permute(2,0,1)
    ...
    def HeatConv(self, u1, u2):
        ...
        K1, K2 = K1.repeat(c, 1, 1), K2.repeat(c, 1, 1)

        KF = torch.exp((-(torch.square(K1)+torch.square(K2)).permute(1,2,0)*self.dt)).permute(2,0,1)
